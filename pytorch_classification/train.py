...
def val():
    ...
    print('val All ACC: %.3f'%(total_correct.float()/(len(val_dataloader) * batch_size)))
...
for iteration in range(start_iter, max_iter):
    ...
    # 在 pytorch 0.4 之后将Variable 与tensor进行合并，所以这里不需要进行Variable封装
    if torch.cuda.is_available():
        images, labels = images.cuda(), labels.cuda()
    ...
    if iteration % 100 == 0:
        print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
              + '|| Totel iter ' + repr(iteration) + ' || Loss: %.6f||' % (loss.item()) + 'TRAIN ACC: %.3f ||' %(train_acc * 100) + 'LR: %.8f' % (lr))
